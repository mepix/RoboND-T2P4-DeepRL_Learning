...
#define TASK8 //Toggle for the final task

#ifdef TASK8
  #define INPUT_WIDTH   128 //Lowered to avoid out of memory error
  #define INPUT_HEIGHT  128 //Lowered to avoid out of memory error
  #define OPTIMIZER "RMSprop"
  #define LEARNING_RATE 0.1f //Reducing value seems to reduce accuracy
  #define REPLAY_MEMORY 10000
  #define BATCH_SIZE 32 //32 seems to be a sweet spot, freezes at 64
  #define USE_LSTM true
  #define LSTM_SIZE 512 //Increasing this helped a lot
#else
  #define INPUT_WIDTH   64 //Lowered to avoid out of memory error
  #define INPUT_HEIGHT  64 //Lowered to avoid out of memory error
  #define OPTIMIZER "RMSprop"
  #define LEARNING_RATE 0.1f //Reducing value seems to reduce accuracy
  #define REPLAY_MEMORY 10000
  #define BATCH_SIZE 32 //32 seems to be a sweet spot, freezes at 64
		...
		*/
...
#ifdef TASK8
		//Reward if we've made contact with the right object :-)
		...
		if(strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0 ||
		   ...
		   strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0
			...
			return;
#endif
		//Punish if we've hit the wrong object :-(
		} else {
...
bool ArmPlugin::updateAgent()
{
	...
	float velocity = (action % 2 == 0) ? (vel[action/2] + actionVelDelta) : (vel[action/2] - actionVelDelta); // Set joint velocity based on whether action is even or odd.
	...
	float joint = (action % 2 == 0) ? (ref[action/2] + actionJointDelta) : (ref[action/2] - actionJointDelta); // Set joint position based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		//bool checkGroundContact = (gripBBox.min.z < groundContact) ? (true) : (false); // End if gripper bounding box is below the ground plane
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				// TODO: compute the smoothed moving average of the delta of the distance to the goal
				const float alpha = 0.9;
				avgGoalDelta  = (avgGoalDelta * alpha) + (distDelta * (1-alpha));
				//rewardHistory = (distGoal == 0) ? (REWARD_WIN) : (REWARD_WIN - avgGoalDelta);
				rewardHistory = avgGoalDelta * 100;
				newReward     = true;
				//endEpisode    = (distGoal == 0) ? (true) : (false);	
			}
